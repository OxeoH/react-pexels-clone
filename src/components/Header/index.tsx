    ...
    const getHeaderPhoto = async () => {
      try {
        const { data } = await axios.get<IPhotosResponse>(
          `https://api.pexels.com/v1/search?page=1&per_page=30&query=nature&orientation=landscape&size=large&color=green`,
          {
            headers: {
              Authorization:
                ...
                <div className={styles.suggested}>
                  ...
                  {pickRandomFromArray(categories, 7).map((category, index) => (
                    <div className={styles.categories}>
                      <Category category={category} lite={true} key={index} />
                      {index === 6 ? "" : ","}
                    </div>
                  ))}
                </div>
